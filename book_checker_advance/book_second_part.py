...
class BookRequest(BaseModel):
    id : Optional[int]= Field(description='ID is not needed when you create a Book' , default=None)
...
@app.get("/books/{inq_id}", status_code=status.HTTP_200_OK)
async def get_book_by_id(inq_id : int = Path(gt=0)):
    for book in BOOKS:
        if book.id == inq_id :
            return book
